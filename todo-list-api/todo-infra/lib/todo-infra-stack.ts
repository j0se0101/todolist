  ...
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const api = new lambda.Function(this, "API", {
      ...
      code: lambda.Code.fromAsset("../api", {
        bundling: {
          // Try local bundling first to avoid Docker
          local: {
            tryBundle(outputDir: string) {
              try {
                // Resolve ../api relative to the CDK working directory (todo-infra)
                const apiDir = path.resolve(process.cwd(), "../api");

                // Try multiple python commands to maximize compatibility
                ...
                let installed = false;
                for (const cmd of pythonInvocations) {
                  try {
                    execSync(`${cmd} install -r requirements.txt -t "${outputDir}"`, {
                      cwd: apiDir,
                      stdio: "inherit",
                    });
                    installed = true;
                ...
                if (!installed) {
                  throw new Error("Failed to install Python dependencies locally");
                }

                // Copy source files into outputDir (Windows: robocopy returns non-zero on success sometimes)
                try {
                  execSync(`robocopy . "${outputDir}" *.* /E /NFL /NDL /NJH /NJS /NC /NS`, {
                    cwd: apiDir,
                    stdio: "inherit",
                  });
                } catch {
